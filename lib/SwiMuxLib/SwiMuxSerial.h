...
class SwiMuxSerial_t {
  ...
  private:
#define UART_DURATION_MS_ROUND(CHAR_COUNT, BAUDS) ((((uint64_t)(CHAR_COUNT) * 10000ULL + ((uint64_t)(BAUDS) / 2ULL)) / (uint64_t)(BAUDS)))
    ...
    static constexpr uint32_t GETUID_TIMEOUT_MS   = (uint32_t)(10 + 2.0 * UART_DURATION_MS_ROUND(10, DEFAULT_SERIAL_BAUDS));
    static constexpr uint32_t READ_TIMEOUT_MS     = (uint32_t)(10 + 2.0 * UART_DURATION_MS_ROUND(140, DEFAULT_SERIAL_BAUDS));
    static constexpr uint32_t WRITE_TIMEOUT_MS    = (uint32_t)(10 + 2.0 * UART_DURATION_MS_ROUND(140, DEFAULT_SERIAL_BAUDS));
    static constexpr uint32_t ROLLCALL_TIMEOUT_MS = (uint32_t)(10 + 2.0 * UART_DURATION_MS_ROUND(sizeof(SwiMuxRespUID_t), DEFAULT_SERIAL_BAUDS));
