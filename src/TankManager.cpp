...
TankManager::TankManager(DeviceState& deviceState, SemaphoreHandle_t& mutex, 
    ...
    : _deviceState(deviceState), _mutex(mutex), _oneWire(oneWire), 
      _eepromController(eepromController), _servoController(servoController) {}
...
bool TankManager::setTankName(const std::string& uid, const std::string& newName) {
    uint8_t addr[8];
    sscanf(uid.c_str(), "%2hhx%2hhx%2hhx%2hhx%2hhx%2hhx%2hhx%2hhx", 
           &addr[0], &addr[1], &addr[2], &addr[3], 
           &addr[4], &addr[5], &addr[6], &addr[7]);
...
void TankManager::_tankDiscoveryTask(void* pvParameters) {
    ...
    for (;;) {
        ...
        ESP_LOGI(TAG, "Running periodic tank discovery...");
...
void TankManager::_discoverAndSyncTanks() {
    ...
    while (_oneWire->search(addr)) {
        ...
        auto it = std::find_if(_knownTanks.begin(), _knownTanks.end(), 
            [&current_uid](const TankInfo& tank){ return tank.uid == current_uid; });
